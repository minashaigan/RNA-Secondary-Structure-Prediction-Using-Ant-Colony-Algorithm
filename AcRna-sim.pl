...
# my($inputfile)=$ARGV[0];
# my($shapefile)=$ARGV[1];
my $inputfile = "adenin.txt"; 
my $shapefile = "adenin.shape"; 
srand(time|$$);
## input : RNA sequence input
	open (my $in,"$inputfile") or die "Can't read $!";
	my $Rna = <$in>;
	$Rna = uc($Rna);
	chomp($Rna);
	$Rna=~ s/\R//g;
...
sub Consistance_stems {
	open(my $out,'>',"D:/Consistance_Stems.txt") or die "Can't open file for writing: $!"; 
	for(my $i=0;$i<scalar(@stems);$i++){
...
sub CompleteEnergy{
	...
	system("./RNAeval --shape=$shapefile  <t1.txt > t2.txt 2> /dev/null");
	#$ES= qx/RNAeval.exe <solutionstem.txt/;
	open(filehandelO3,"t2.txt") or die("error");
	<filehandelO3>;
	...
	for(my $i=0;$i<scalar(@ants_solution);$i++){
		for(my $j=0;$j<scalar(@{$ants_solution[$i]});$j++){
			for(my $k=$j+1;$k<scalar(@{$ants_solution[$i]});$k++){
					...
					#$$pheremones[$ants_solution[$i][$j]][$ants_solution[$i][$k]] += $energy/$min_energy;
					#$pheremones[$ants_solution[$i][$k]][$ants_solution[$i][$j]] += $energy/$min_energy;	
				#	if($min_energy>=$energy){
				#	$pheremones[$ants_solution[$i][$j]][$ants_solution[$i][$k]] += 0.9;
				#	$pheremones[$ants_solution[$i][$k]][$ants_solution[$i][$j]] += 0.9;	
				#	}
				#	else{
				#		$pheremones[$ants_solution[$i][$j]][$ants_solution[$i][$k]] += 0.1;
				#	$pheremones[$ants_solution[$i][$k]][$ants_solution[$i][$j]] += 0.1;	
				#	}
...
sub print_result{
	...
	print $out "TP : ",$TP = $TP/2,"	FP : ",$FP = $ppv2-$TP," FN : ",$FN = $se2-$TP,"\n";
	
	
	print $out "sensitivity : ",$TP/($TP+$FN),"\n";
	
	print $out "PPV : ",$TP/($TP+$FP),"\n";
	
	print $out "f-measure : ",(2*($TP/($TP+$FN))*($TP/($TP+$FP)))/(($TP/($TP+$FN))+($TP/($TP+$FP))),"\n";
