...
sub main {
	open (my $in,"D:/DESKTOP/RNA-data/5s.txt") or die "Can't read $!";  
	my $RNA = <$in>;
	$RNA = uc($RNA);
	chomp($RNA);
	$RNA=~ s/\R//g;
	...
	while( $i <= 2 )
	{
		...
		# @sub_solution = Update_sub_solution(\@sub_solution, \@ants_solution, \@Stems, $RNA, $min_energy);
		...
		if( !$check_sub_solution ){
			...
			$i=0;
			print $out $sub_solution = Structure(\@sub_solution, \@Stems, length($RNA));
...
sub Select_Next_Stem {
	...
	for(my $i=0;$i<scalar(@allowed);$i++){
		...
		if( $consistent ){
			...
			# $sum += $pheremones[$current_stem][$allowed[$i]]*$hi[$current_stem][$allowed[$i]];
		}
	}
	my $x0 = rand(1);
	my $rnd =  4*$x0*(1-$x0);
