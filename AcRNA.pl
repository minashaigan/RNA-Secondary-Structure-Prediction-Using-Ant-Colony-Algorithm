...
sub main {
	open (my $in,"D:/DESKTOP/RNA-data/adenin.txt") or die "Can't read $!";  
	my $RNA = <$in>;
	$RNA = uc($RNA);
	chomp($RNA);
	$RNA=~ s/\R//g;
	...
	while( $i <= 50 )
	{
		...
		$min_energy = Update_min_energy(\@ants_solution, \@Stems, $RNA);
		@sub_solution = Update_sub_solution(\@ants_solution, \@Stems, $RNA, $min_energy);
		
		
		@Pheremones = Update_Pheremone(\@Pheremones, \@ants_solution, \@Stems, $min_energy, $RNA);
		
			
		my $check_sub_solution = Change_Sub_Solution(\@sub_solution, \@ants_solution, \@Stems, $RNA);
		if( !$check_sub_solution ){
			$i++;
		}
	}
	print $min_energy,"\n";
	my $best_Structure = Best_Solution(\@sub_solution, \@Stems, $RNA);
	print_result($Structure, $best_Structure, $RNA, $IL);
...
sub Select_Next_Stem {
	...
	for(my $i=0;$i<scalar(@allowed);$i++){
		$probabilty_current_stem += ($pheremones[$current_stem][$allowed[$i]]*$pheremones[$current_stem][$allowed[$i]]*$hi[$current_stem][$allowed[$i]])/$sum;
	...
	return (\@allowed,\@solution,scalar(@allowed));
}
...
sub Update_Pheremone{
	...
	for(my $i=0;$i<scalar(@ants);$i++){
		...
		my $structure = Structure(\@solution, \@stems, length($RNAseq));
		my $energy = CompleteEnergy($structure, $RNAseq);
		for(my $n=0;$n<scalar(@solution);$n++){
			for(my $m=$n+1;$m<scalar(@solution);$m++){
...
sub Change_Sub_Solution {
	...
	my $flag;
	if( scalar(@subs) == scalar(@ants) ){
		for(my $i=0;$i<scalar(@subs);$i++){
			$flag = 0;
			my @sub_solution = @{$subs[$i]};
			my $sub_structure = Structure(\@sub_solution, \@stems, length($RNAseq));
			for(my $j=0;$j<scalar(@ants);$j++){
				my @ant_solution = @{$ants[$i]};
				my $ant_structure = Structure(\@ant_solution, \@stems, length($RNAseq));
				if( $sub_structure eq  $ant_structure ){
					$flag = 1;
					last;
				}
			}
