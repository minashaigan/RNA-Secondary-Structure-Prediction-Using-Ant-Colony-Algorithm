...
# open (my $in,"D:/DESKTOP/RNA-data/HCV-domain2.txt") or die "Can't read $!";  
...
sub print_pheremones_inFile {
	...
	for(my $i=0;$i<scalar(@stems);$i++)
	{
		...
		{
			print $out $pheremones[$i][$j]*$hi[$i][$j]*$hi[$i][$j],"\n";
		}
	}
	close $out or die "Failed to close file: $!";
}
	...
	for(my $i=0;$i<scalar(@stems);$i++){
		for(my $j=0;$j<scalar(@stems);$j++){
			...
			# if( $consistent == 1){
				...
				# }
				# else{
					# $hi[$i][$j] = ($energy_current_stem*$energy_next_stem + $stems[$i]{u3}*$stems[$j]{u3} )/( sqrt( ($d1*$d1) + ($d2*$d2) ) );
				# }
			# }
			# if( $consistent == 3){
			...
			# if( $consistent == 2 ){
				...
				# }
				# else{
					# $hi[$i][$j] = ($energy_current_stem*$energy_next_stem + $stems[$i]{u3}*$stems[$j]{u3} )/$d3;
				# }
			# }
			# if( $consistent == 4 ){
