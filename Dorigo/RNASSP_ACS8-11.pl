...
open (my $in,"D:/.RNA Secondary Structure Prediction Using Ant Colony Algorithm/Data/RNA-data/5s.txt") or die "Can't read $!";   
my $RNA = <$in>;
$RNA = uc($RNA);
chomp($RNA);
$RNA=~ s/\R//g;
...
sub Heuristic_Information {
	...
	for(my $i=0;$i<scalar(@stems);$i++){
		for(my $j=0;$j<scalar(@stems);$j++){
			...
			my $hi_value;
			if($energy_next_stem>=0 or $energy_current_stem>=0){
				$hi_value = 0;
			}
			else{
				if($energy_next_stem<$energy_current_stem){
					$hi_value = $energy_next_stem*$energy_current_stem + $energy_next_stem/$energy_current_stem;
				}
				else{
					$hi_value = $energy_next_stem*$energy_next_stem/$energy_current_stem;
				}
			}
			# if($energy_next_stem<0 and $energy_current_stem<0){
				# $hi_value = $energy_next_stem*$energy_next_stem/-$energy_current_stem;
			# }
			# else{
				# $hi_value = 0;
			...
			if( $consistent == 1){
				...
				if($d1 == 0 and $d2 == 0){
					...
					$hi[$i][$j] = 1/( sqrt( ($d1*$d1) + ($d2*$d2) ) ) + $hi_value + ($stems[$j]{u3}*$stems[$j]{u3})/$stems[$i]{u3} ;
				}
			}
			if( $consistent == 3){
			...
			if( $consistent == 2 ){
				...
				if($d3 == 0){
					...
					$hi[$i][$j] = 1/$d3 + $hi_value + ($stems[$j]{u3}*$stems[$j]{u3})/$stems[$i]{u3};
				}
			}
			if( $consistent == 4 ){
...
#print pheremones in file
sub print_hi_inFile {
	open(my $out,'>',"hi.txt") or die "Can't open file for writing: $!"; 
	for(my $i=0;$i<scalar(@stems);$i++)
	{
		for(my $j=0;$j<scalar(@stems);$j++)
		{
