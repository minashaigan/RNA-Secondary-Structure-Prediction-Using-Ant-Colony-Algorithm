...
sub Heuristic_Information {
	...
	for(my $i=0;$i<scalar(@stems);$i++){
		for(my $j=0;$j<scalar(@stems);$j++){
			...
			if( $consistent ){
				# $hi[$i][$j] =  (-1*$energy_current_stem*$stems[$i]) + (-1*$energy_next_stem*$stems[$j]);
				# $hi[$i][$j] =  ($stems[$j]*$stems[$j]/$stems[$i]) * ( $energy_next_stem*$energy_next_stem/$energy_current_stem ) * -1;
				# $hi[$i][$j] =  ($stems[$j]*$stems[$j]/$stems[$i]);
				# $hi[$i][$j] =  ( ($energy_next_stem*$energy_next_stem*$stems[$j]*$stems[$j])/($energy_current_stem*$stems[$i]*$stems[$i]) ) * -1;
				$hi[$i][$j] = -1 * ($energy_current_stem+$energy_next_stem);
			}
			if( $consistent == 0 ){
				$hi[$i][$j] = 0;
			}
