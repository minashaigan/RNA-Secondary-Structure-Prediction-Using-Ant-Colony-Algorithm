...
sub Select_Next_Stem {
	...
	if( $q_rnd <= $q and scalar(@allowed)){
		...
		for(my $i=1;$i<scalar(@allowed);$i++){
			if( $max < $pheremones[$current_stem][$allowed[$i]]*$hi[$current_stem][$allowed[$i]]*$hi[$current_stem][$allowed[$i]] ){
				$max_allowed = $allowed[$i];
				$max = $pheremones[$current_stem][$allowed[$i]]*$hi[$current_stem][$allowed[$i]]*$hi[$current_stem][$allowed[$i]];
			}
		}
