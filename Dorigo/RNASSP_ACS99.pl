...
open (my $in,"D:/.RNA Secondary Structure Prediction Using Ant Colony Algorithm/Data/RNA-data/trna-phe.txt") or die "Can't read $!";  
my $RNA = <$in>;
$RNA = uc($RNA);
chomp($RNA);
$RNA=~ s/\R//g;
...
sub Heuristic_Information {
	...
	for(my $i=0;$i<scalar(@stems);$i++){
		for(my $j=0;$j<scalar(@stems);$j++){
			...
			## good answer adenin
				if( $consistent){
					$hi[$i][$j] =  ( $energy_next_stem*$energy_next_stem/$energy_current_stem ) + ( $stems[$j]{u3}*$stems[$j]{u3}/$stems[$i]{u3} );
				}
				if( $consistent == 0 ){
					$hi[$i][$j] = 0;
				}
			...
			# }
			# if( $consistent){
				# $hi[$i][$j] = ( $energy_current_stem*$energy_next_stem + $stems[$i]{u3}*$stems[$j]{u3}) ;
				# $hi[$i][$j] = ( $energy_current_stem*$energy_next_stem ) ;
				# $hi[$i][$j] = ( $energy_current_stem+$energy_next_stem) ;
			# }
			# $hi[$i][$j] =  ($energy_current_stem*$energy_next_stem + $stems[$i]{u3}*$stems[$j]{u3} - 2.17 ) ;
			# if( $consistent == 1){
				# my $d1 = $stems[$j]{u1} - ( $stems[$i]{u1} + $stems[$i]{u3} );
				# my $d2 = ( $stems[$i]{u2} - $stems[$i]{u3} ) - $stems[$j]{u2};
				# if($d1 == 0 and $d2 == 0){
					# $hi[$i][$j] =  ($energy_current_stem*$energy_next_stem + ($stems[$j]{u3}*$stems[$j]{u3}/$stems[$i]{u3}) ) ;
				# }
				# else{
					# $hi[$i][$j] = ( ($energy_current_stem*$energy_next_stem + ($stems[$j]{u3}*$stems[$j]{u3}/$stems[$i]{u3}) )/( sqrt( ($d1*$d1) + ($d2*$d2) ) ) ) ;
				# }
			# }
			# if( $consistent == 3){
			...
			# if( $consistent == 2 ){
				...
				# }
				# else{
					# $hi[$i][$j] = ( ($energy_current_stem*$energy_next_stem + ($stems[$j]{u3}*$stems[$j]{u3}/$stems[$i]{u3}))/$d3 ) ;
				# }
			# }
			# if( $consistent == 4 ){
